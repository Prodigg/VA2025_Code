
#include "pindef.h"
#include "config.h"

#include <Adafruit_NeoPixel.h>
#include <PS2X_lib.h>
#include <TMCStepper.h>
#include <AccelStepper.h>

Adafruit_NeoPixel pixels(4, pins::neopixelPin, NEO_GRB + NEO_KHZ800);
PS2X ps2x;

TMC2209Stepper driverM0 = TMC2209Stepper(&pins::M0_SERIAL_PORT, R_SENSE, DRIVER_ADDRESS);
TMC2209Stepper driverM1 = TMC2209Stepper(&pins::M1_SERIAL_PORT, R_SENSE, DRIVER_ADDRESS);

AccelStepper stepperM0 = AccelStepper(stepperM0.DRIVER, pins::M0_STEP_PIN, pins::M0_DIR_PIN);
AccelStepper stepperM1 = AccelStepper(stepperM1.DRIVER, pins::M1_STEP_PIN, pins::M1_DIR_PIN);
AccelStepper stepperM2 = AccelStepper(stepperM2.DRIVER, pins::M2_STEP_PIN, pins::M2_DIR_PIN);

void setup() {
    pinMode(pins::buttonPin, INPUT);
    pixels.begin();
}

void loop() {
    for (int i = 0; i < 4; i++) {
        if (digitalRead(pins::buttonPin) == HIGH) {
            pixels.setPixelColor(i, pixels.Color(0, 0, 255));
        }
        else
            pixels.setPixelColor(i, pixels.Color(255, 0, 0));
    }
    pixels.setBrightness(100); 

    pixels.show();
}
